...
(defn cart-item [product] 
   [:div 
    {:class "cf w5"}
    [:span {:class "fl"} (concat (str (product :id)) ": ")]
    [:span {:class "fr"} (concat "$" (str (product :price)))] ])

(defn cart []
   (let [order (subscribe [::subs/order])
         user  (subscribe [::subs/user])]
   [:div
     [:h2 (concat "Welcome back " ( :first_name @user) " " (:last_name @user))]
  
     [:h4 "your cart product ids:"]
     [:div 
      (for [product @order]
           (cart-item product))]
  
     [:button 
      {:on-click #(dispatch [::events/load-initial-data])}
       "reload cart"]]))
